...
class TestMovingAverageSignal:
    ...
    def test_hold_when_flat(self):
        closes = [100.0] * 25
        assert moving_average_signal(closes, fast=5, slow=20) == "hold"

    def test_buy_crossover(self):
        closes = [100.0] * 24 + [200.0]
        assert moving_average_signal(closes, fast=5, slow=20) == "buy"

    def test_sell_crossover(self):
        closes = [100.0] * 24 + [10.0]
        assert moving_average_signal(closes, fast=5, slow=20) == "sell"
