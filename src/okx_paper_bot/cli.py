...
def main() -> None:
    ...
    parser.add_argument("--symbol", type=str, help="交易对 (覆盖 .env)")
    parser.add_argument("--timeframe", type=str, help="K线周期 (覆盖 .env)")
    parser.add_argument("--stop-loss", type=float, help="止损百分比 (0.05 = 5%%)")
    parser.add_argument("--take-profit", type=float, help="止盈百分比 (0.10 = 10%%)")
    ...
    if args.stop_loss is not None:
        overrides["stop_loss_pct"] = args.stop_loss
    if args.take_profit is not None:
        overrides["take_profit_pct"] = args.take_profit
    if args.trailing_stop is not None:
        overrides["trailing_stop_pct"] = args.trailing_stop
    if args.interval is not None:
        overrides["loop_interval_seconds"] = args.interval
    if overrides:
    ...
    if args.run:
        ...
        closes = fetch_close_prices(
            exchange, symbol=config.symbol, timeframe=config.timeframe,
